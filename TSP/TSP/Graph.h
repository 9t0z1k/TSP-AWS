...
void Reduction(int*** matrix, int VN)
{
	...
	for (int i = 0; i < VN; i++)
	{
		for (int j = 0; j < VN; j++) 
			if (matrix[i][j] && min > *matrix[i][j])
				min = *matrix[i][j];
		for (int j = 0; j < VN; j++) // Редукция строк
			if (matrix[i][j])
				*matrix[i][j] -= min;
	}
	...
	for (int j = 0; j < VN; j++)
	{
		for (int i = 0; i < VN; i++)
			if (matrix[i][j] && min > *matrix[i][j]) 
				min = *matrix[i][j];
		for (int i = 0; i < VN; i++) // Редукция столбцов
			if (matrix[i][j])
				*matrix[i][j] -= min;
	}
}
...
void BranchAndBound(int*** matrix, int VN, int** zeroComp, int* wayop)
{
	for (int l = 0; l < VN; l++)
	{
		...
		for (int i = 0; i < VN; i++) // Нахождение оценок для нулевых элементов
			for (int j = 0; j < VN; j++)
			{
				if (matrix[i][j] && !*matrix[i][j])
				{
					...
					zeroComp[i][j] = TMPmin + TMP2min; // Оценка
				}
			}

		int mcost = 0, mi = 0, mj = 0;
		for (int i = 0; i < VN; i++) // Выбор нулевого элемента с наибольшей оценкой
			for (int j = 0; j < VN; j++)
		...
		for (int i = 0; i < VN; i++) // Редукция матрицы по столбцам
			matrix[i][mj] = nullptr;

		for (int i = 0; i < VN; i++) // Редукция матрицы по строкам
			matrix[mi][i] = nullptr;

		matrix[mj][mi] = nullptr; // Убираемм нулевой элемент
		printMatrix(matrix, VN);
	}
}
...
void Graph<T>::TSP()
{
	...
	for (int i = 0; i < vertexNum; i++) // Копия матрицы смежности для последующих редукций
	{
...
void Graph<T>::visualizeGraph(Graph<T>& graph) {
	...
	// Кнопки для создания и удаления вершин
	sf::RectangleShape addButton(sf::Vector2f(150, 50));
	...
	sf::RectangleShape deleteButton(sf::Vector2f(150, 50));
	deleteButton.setPosition(10.0f, 70.0f);
	...
	// Кнопка для создания вершины
	sf::RectangleShape addEdgeButton(sf::Vector2f(150, 50));
	addEdgeButton.setPosition(sf::Vector2f(10.0f, 140.0f));
	addEdgeButton.setFillColor(sf::Color::Black);
	...
	//Кнопка для печати матрицы смежности
	sf::RectangleShape addPrintMatrixButton(sf::Vector2f(150, 50));
	addPrintMatrixButton.setPosition(10.0f, 210.0f);
	addPrintMatrixButton.setFillColor(sf::Color::Magenta);
	...
	//TSP button
	sf::RectangleShape addTSPButton(sf::Vector2f(150, 50));
	addTSPButton.setPosition(10.0f, 280.0f);
	addTSPButton.setFillColor(sf::Color::Cyan);
